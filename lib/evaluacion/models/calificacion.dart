...
class Calificacion {
  ...
  // Getter para evaluación (si lo usas)
  String get evaluacionId => id;

  factory Calificacion.fromMap(Map<String, dynamic> map) {
    List<String> sistemasDesdeMapa = [];
    final raw = map['sistemas'];
    if (raw is List) {
      sistemasDesdeMapa = raw.map((e) => e.toString()).toList();
    } else if (raw is String && raw.isNotEmpty) {
      try {
        final decoded = jsonDecode(raw);
        if (decoded is List) {
          sistemasDesdeMapa = decoded.map((e) => e.toString()).toList();
        } else {
          sistemasDesdeMapa = [decoded.toString()];
        }
      } catch (_) {
        sistemasDesdeMapa = [raw];
      }
    }
