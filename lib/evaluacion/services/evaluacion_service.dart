...
class EvaluacionService {
  ...
  Future<List<Evaluacion>> getEvaluaciones() async {
    final res = await _client.from('calificaciones').select();
  ...
  Future<void> updateEvaluacion(String id, Evaluacion evaluacion) async {
    await _client.from('calificaciones').update(evaluacion.toMap()).eq('id', id);
  }
  ...
  Future<Evaluacion> crearEvaluacionSiNoExiste(String empresaId, String asociadoId) async {
    ...
    await _client.from('calificaciones').insert(nueva.toMap());
    return nueva;
  }

  Future<void> guardarEvaluacionDraft(String evaluacionId) async {
    await _client.from('calificaciones').update({'finalizada': false}).eq('id', evaluacionId);
  }

  Future<void> finalizarEvaluacion(String evaluacionId) async {
    await _client.from('calificaciones').update({'finalizada': true}).eq('id', evaluacionId);
  }
